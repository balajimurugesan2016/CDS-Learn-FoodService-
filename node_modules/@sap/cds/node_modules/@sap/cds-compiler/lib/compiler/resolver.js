...
function resolve( model ) {
  ...
  const { initArtifacts, lateExtensions, hasTruthyProp } = getDefinerFunctions(model);
  let newAutoExposed = [];
  return main();
  ...
  function main() {
    ...
    createLocalizedConvenienceViews();
    // Phase 8: apply ANNOTATE on autoexposed entities and unknown artifacts:
  ...
  function resolveUsings( src, topLevel ) {
    ...
    for (const def of src.usings) {
      if (def.usings)           // using {...}
        resolveUsings( def );
      if (!def.name || !def.name.absolute) // using {...} or parse error
        continue;
   ...
   */
  function createLocalizedConvenienceViews() {
    ...
    const localizedCache = Object.create(null);

    // step 1: create localized convenience views for entities
    const convenienceViews = Object.create(null); // cache convenience views for entities
    ...
    for (const viewName in convenienceViews) {
      const info = convenienceViews[viewName];
      initializeLocalizedConvenienceView(viewName, info.view, info.base, convenienceViews);
    }
    // step 1.3: execute compile phases that have already been done for other
    // artifacts on conv views
    for (const viewName in convenienceViews) {
      const { view } = convenienceViews[viewName];
    ...
    const views = initializeLocalizedConvenienceViewsForViews(model._entities, localizedCache);
    // step 2.2: rewrite _artifact, _navigation and _finalType properties
  ...
  function isLocalizedOrAssocToLocalized(artifact, localizedCache) {
    ...
    function recurse(art) {
      ...
      for (const elemName in art.elements) {
        const element = art.elements[elemName];
  ...
  function prepareLocalizedConvenienceView(results, localizedCache) {
    return function create(art) {
      ...
      if (art.kind === 'entity' && art.elements && (!art.abstract || !art.abstract.val) && !art.query && isLocalizedOrAssocToLocalized(art, localizedCache)) {
        ...
        const view
              = createLocalizedDataView( art, viewName, art.$localizedElements || [], results );
  ...
  function createLocalizedDataView( base, absolute, textElems ) {
    ...
    if (base._service)          // make implicit redirection work
      setLink( art, base._service, '_service' );
    addBoolAnnotationTo('@odata.draft.enabled', false, art);

    for (const localizedElem of textElems) {
      const origElement = localizedElem.elem;
      const { assoc } = localizedElem;
      const assocName = (assoc === 'default') ? 'localized' : assoc.name.id;
      if (!origElement.key || !origElement.key.val) {
        // eslint-disable-next-line no-shadow
        const { location } = origElement.name;
        const { id } = origElement.name;               // element name in the texts entity
        const newId = localizedElem.newNameId || id;  // element name in the view / projection
        ...
        columns.push( {
          name: { id: newId, location },
          location: origElement.location,
          value,
          origin,
          $replacement: 'silent',
        } );
  ...
  function initializeLocalizedConvenienceView(viewName, view, base, convenienceViews) {
    // TODO: re-check locations
    const { location } = base;
    // rewrite associations
    for (const elemName in base.elements) {
      const origElem = base.elements[elemName];
      // skip any non-association
      if (!origElem.target || !origElem.target._artifact)
        continue;
      const localizedName = `localized.${ origElem.target._artifact.name.absolute }`;
      // skip if no localized target exists
      if (!convenienceViews[localizedName])
        continue;
      // rewrite association to localized target
      const value = { path: [ { id: origElem.name.element, location } ], location };
      const targetPath = [ { id: localizedName, location } ];
      view.query.columns.push({
        name: { id: origElem.name.id, location: origElem.name.location },
   ...
   * These convenience views are a simple copy of the original view with a new name
   * ('localized.' + originalName).  There isn't any redirection for them
   * to their corresponding localized artifacts as this has to happen
  ...
  function initializeLocalizedConvenienceViewsForViews( entities, localizedCache ) {
    ...
    const localizedEntities = entities.filter((art) => {
      ...
      const isLocalized = dsome(art.elements, (elemName) => {
    ...
    return localizedEntities.map((art) => {
      ...
      // - copy query
      const query = copyExpr(art.query, location, true, true);
      // - create view
      const localizedView = {
        kind: 'entity',
      ...
      if (art._service)         // make implicit redirection work
        setLink( localizedView, art._service, '_service' );

      // annotate convenience views with @odata.draft.enabled: false as they
      // aren't part of any service
  ...
  function rewriteToLocalized(art) {
    ...
    if (proto && proto !== Object.prototype) // do not copy object from special classes
      return;
    for (const prop of Object.getOwnPropertyNames(art)) {
      if (prop === '_artifact' || prop === '_navigation' || prop === '_finalType' ) {
        const linkedArt = art[prop];
        if (!linkedArt)
          continue;

        const localizedArt = getLocalizedVariant(linkedArt);
        if (localizedArt) {
          const descr = Object.getOwnPropertyDescriptor(art, prop);
  ...
  function getLocalizedVariant(art) {
    ...
    const mainArtName = art.name.absolute;
    if (!mainArtName)           // magic variable
      return art;
    if (mainArtName.startsWith('localized.'))
      return null;
    const localizedName = `localized.${ mainArtName }`;
    ...
    if (localizedArt) { // if localized main artifact exists
      ...
      if (art.name.element) {
        ...
        const elemNames = art.name.element.split('.');
        for (const elemName of elemNames) {
          // depending on the type of the sub artifact we have to take a look
          // at different collections.
          let collection = localizedArt.elements ||
              localizedArt.$navigation ||
              localizedArt.foreignKeys ||
              localizedArt.enum;
          // In case of redirected associations we have to follow them as they
          // don't have elements nor $navigation property
          if (!collection) {
            const target = getLocalizedVariant(localizedArt.target._artifact) ||
                  localizedArt.target._artifact;
            collection = target.elements;
          }
          localizedArt = collection[elemName];
        }
      }
      // This internal error triggers if we found a localized variant for the
      // main artifact but the algorithm above wasn't able to follow the path
      // to get to the sub artifact.
      if (!localizedArt)
        throw Error('Unexpected error when rewriting localized convenience view');
    }
    return localizedArt;
  }
  ...
  function resolveTarget( art, obj ) {
    ...
    if (obj.onCond) {
      if (!art._main || !art._parent.elements) {
        ...
        resolveExpr( obj.onCond, 'unmanaged', art );
      }
    }
