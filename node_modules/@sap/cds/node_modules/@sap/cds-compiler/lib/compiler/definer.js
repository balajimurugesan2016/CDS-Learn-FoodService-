...
function getDefinerFunctions( model ) {
  ...
  function processCompositionOfType( base ) {
    ...
    function expand( elem ) {
      if (!elem.target || elem.key && elem.key.val || elem.onCond || elem.foreignKeys || !elem.type)
        return;
      const type = elem.type.path;
      if (!type || !type[0] || type[0].id !== 'cds.Composition')
        return;
      let { target } = elem;
      if (!target.elements) {
        target = resolvePath( elem.target, 'compositionTarget', elem );
