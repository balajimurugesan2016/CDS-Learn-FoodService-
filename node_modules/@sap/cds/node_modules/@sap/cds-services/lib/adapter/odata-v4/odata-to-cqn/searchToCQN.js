...
const _handleContains = (contains, columns, searchTerm, fuzzyIndex) => {
  return [
    {
      func: contains,
      ...
      args: [
        ...
        { func: 'fuzzy', args: [{ val: fuzzyIndex }, { val: 'similarcalculationmode=substringsearch' }] }
      ]
    }
  ]
}
 ...
 * @param fuzzyIndex - fuzzy score between 0 and 1, default 0.75
 ...
 */
const searchToCQN = (columnList, search, fuzzyIndex) => {
  switch (search.getKind()) {
    case BINARY:
      const operator = search.getOperator().toLowerCase()
      const left = _searchRecursive(columnList, search.getLeftOperand())
      const right = _searchRecursive(columnList, search.getRightOperand())
      return _handleContains('contains', columnList, [...left, operator, ...right], fuzzyIndex)
    case LITERAL:
      return _handleContains('contains', columnList, `${search.getText()}`, fuzzyIndex)
    case UNARY:
      return _handleContains('not contains', columnList, `${search.getOperand().getText()}`, fuzzyIndex)
    default:
      throw new FeatureNotSupported(`Search expression ${_getExpressionKindName(search.getKind())} is not supported`)
  }
}
