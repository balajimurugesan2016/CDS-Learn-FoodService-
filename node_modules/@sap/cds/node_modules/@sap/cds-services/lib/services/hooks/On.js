...
class On extends Base {
  ...
  _handleAsyncHandler (result, context, handler, nextOnce, reject, oldTarget) {
      ...
      result
        ...
        .catch(err => {
          ...
          if (!handler.handler.isDefault) {
            err.message = `Error at custom event handler ${handler.event}:
${err.message}`
          }
          reject(err)
        })
      ...
      while (i < length) {
        ...
        if (this._match(event, context, handler)) {
          ...
          try {
            ...
            this._handleAsyncHandler(result, context, handler, nextOnce, reject, oldTarget)
            return this._handleLayerReturn(context, result, resolve, reject, calledFromHandler === -1)
          } catch (err) {
            // Do not measure performance in case of error
            context.target = oldTarget
            if (!handler.handler.isDefault) {
              err.message = `Error at custom event handler ${event}:
${err.message}`
            }
            reject(err)
            return
   ...
   * @private
   */
  _handleLayerReturn (context, layerReturn, resolve, reject, isFirstHandler) {
    // Promise or equaly behaving thenables.
    if (layerReturn && layerReturn.then) {
      return this._handleThenables(context, layerReturn, resolve, reject, isFirstHandler)
    }

    // CQN means, we should run it
    if (this._isCQN(layerReturn)) {
      return this._handleThenables(context, context.run(layerReturn), resolve, reject, isFirstHandler)
    }
   ...
   * @private
   */
  _handleThenables (context, promise, resolve, reject, isFirstHandler) {
    return promise
      .then(result => {
        if ('rejected' in context) {
