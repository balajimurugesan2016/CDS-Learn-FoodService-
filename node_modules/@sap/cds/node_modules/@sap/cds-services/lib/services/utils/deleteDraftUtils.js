...
const _getSelectCQN = (context, keys) => {
  return context.statements.SELECT.from(ensureNoDraftsSuffix(context.target.source), [1]).where(keys.keyList)
}
...
const _getActiveDeleteCQN = ({ statements: { DELETE } }, entity, keys) =>
  DELETE.from(ensureNoDraftsSuffix(entity.source)).where(...keys)
