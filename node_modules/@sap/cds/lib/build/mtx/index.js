...
class MtxModuleBuilder extends BuildTaskHandler {
    ...
    async build() {
        ...
        if (tasks.length === 0 || !tasks.find(task => task.for === BUILD_TASK_HANA)) {
            const dbKind = BuildTaskHandler._getProperty(this.env, ["requires", "db", "kind"])
            ...
            if (dbKind !== "hana") {
                ...
                BuildTaskHandler._setProperty(this.env, ["requires", "db", "kind"], "hana")
            }
            try {
                derivedTasks = await buildTaskFactory._createTasksFromCdsConfig(this.buildOptions.root)
            } finally {
                if (dbKind !== "hana") {
                    BuildTaskHandler._setProperty(this.env, ["requires", "db", "kind"], dbKind)
                }
            }
            if (tasks.length === 0) {
    ...
    async _copyNativeHanaContent(src, dest) {
        const regex = /\.csv$|\.cds$|\.hdb.*$|^\.hdi.*|^undeploy\.json/
        ...
        return super._copyNativeContent(src, dest, (entry) => {
            ...
            const match = path.basename(entry).match(regex)
    ...
    async _copyTemplates(src, dest) {
        return super._copyNativeContent(src, dest, (entry) => {
            if (fs.statSync(entry).isFile()) {
                const match = path.basename(entry).match(/\.cds$/)
                return Array.isArray(match) && match.length > 0
            }
            return true
        })
