// const cds = require('../cds'), cdsv = _emitting ( cds._compiler ) --> s.u.
const cds = require('../cds'), cdsv = cds._compiler
const { cdsc, sql_mapping } = cds.env,  _ = (...options) => Object.assign ({}, cdsc, ...options)
const _hanaOpts = o => { return {toHana:{ csn: (o.csn !== undefined ? o.csn : false), src: (o.src !== undefined ? o.src : true), names: o && o.sql_mapping || sql_mapping, associations: 'assocs' }} }
...
function _add_source_map_to_services_in(csn) { // NOSONAR
  ...
  for (let each of Object.values (csn.definitions)) { if (each.kind === "service") {
    ...
    if (each.$location && each.$location.file) {
      each["@source"] = each.$location.file
    }
  }}
}
...
// REVISIT: Warum machten wir das?
// Wir haben eh schon zunehmend massive performance probleme, da sollten
// wir alles zusätliche vermeiden. Und bitte, lasst uns damit aufhören, immer
// wieder kern-module für spezifische verwender, wie CLI oder tests aufzuboren.
// Das ist das Gegenteil von Modularisierung.
//
