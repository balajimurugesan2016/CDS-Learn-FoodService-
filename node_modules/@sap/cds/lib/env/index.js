...
//
  ...
  _add_to_process_env (cwd, filename) {
    ...
    for (const key in json) {
      ...
      const val = json[key]
      process.env[key] = typeof val === 'object' ? JSON.stringify(val) : String.prototype.toString.call(val)
    }
    this._sources.push (file)
  }

  _link_required_services () {
    const { requires, profiles } = this
    if (requires)  Object.entries (requires) .forEach (([key,entry]) => {
      if (key === entry.kind) return // avoid endless recursion
      requires[key] = _merged (entry)
    })
    function _merged (entry) {
      const base = requires [entry.kind]
      return !base ? entry : _merge (entry, _merged(base), profiles)
    }
  }
...
//
  ...
  _for_tests (...conf) {
    ...
    for (let c of conf)  _merge(env, c)
 ...
 * @returns {Config} dst
 */
function _merge (dst, src, _profiles) { // NOSONAR
  for (let p in src) {
    const v = src[p]
    if (_profiles && p[0] === '[') {
      if (_profiles.includes(p.slice(1,-1)))  _merge (dst, v, _profiles)
      delete dst[p]
    }
