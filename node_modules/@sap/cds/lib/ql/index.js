...
extend (ANY) .with ({
    ...
    then(r,e) {
        const srv = !this._target.service ? cds.db : cds.connect.to (this._target.service.name)
...
// Enable linked cds.entity definitions as cds.ql-thenable targets
cds.entity.mixin (class {
    get service(){
        const value = this.parent && this.parent.kind === 'service' && this.parent
...
const { UPDATE } = ql
const UPDATE_ = _smart (UPDATE)
const UPDATE_prototype = UPDATE('x').__proto__
