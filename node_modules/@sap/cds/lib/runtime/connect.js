...
function cds_connect (options) {
	...
	cds.db = cds_connect_to (options || 'db',undefined,  true)
	return cds
}
...
function cds_connect_to (datasource, options, ___primary) {
	...
	if (o.primary || ds === 'db')  cds.db = promised
...
function _options4 (datasource, _options) { // NOSONAR
	const conf = cds.env.requires [datasource]
	const o = { ...conf, ..._options }
	const [, kind, url ] = /^(\w+):(.*)/.exec(datasource) || []
	if (kind)  return ['', { ...o, kind, credentials: { url, database:url } }]
	if (!o.kind) {
		o.kind = datasource === 'sqlite' ? datasource : o.driver
		if (!o.kind && !o.silent) { // still not --> provide a good error message...
			if (!conf && datasource) _error (`didn't find a configuration for 'cds.requires.${datasource}'`)
			if (conf)  _error (`configuration for 'cds.requires.${datasource}' lacks mandatory property 'kind'`)
